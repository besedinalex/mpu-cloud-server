...
export async function createUserFolder(id: number, currentPath: string, folderName: string, response: ServiceResponse) {
    currentPath = `/u${id}/${currentPath}/`;
...
export async function copy(id: number, currentPath: string, newPath: string, response: ServiceResponse) {
    currentPath = `u${id}/${currentPath}`;
    newPath = `u${id}/${newPath}`;
    if (FileManager.getFullPath(`/u${id}/`) === FileManager.getFullPath(currentPath)) {
        response(400, {message: 'Нельзя скопировать корневую папку пользователя.'});
        return;
    }
    if (!FileManager.pathExists(currentPath)) {
...
export async function replace(id: number, currentPath: string, newPath: string, response: ServiceResponse) {
    currentPath = `u${id}/${currentPath}`;
    newPath = `u${id}/${newPath}`;
    if (FileManager.getFullPath(`/u${id}/`) === FileManager.getFullPath(currentPath)) {
        response(400, {message: 'Нельзя переместить корневую папку пользователя.'});
        return;
    }
    if (!FileManager.pathExists(currentPath)) {
...
export async function rename(id: number, currentPath: string, currentName: string, newName: string,
                             response: ServiceResponse) {
    currentPath = `/u${id}/${currentPath}/`;
...
export async function remove(id: number, path: string, response: ServiceResponse) {
    path = `/u${id}/${path}/`;
    if (FileManager.getFullPath(`/u${id}/`) === FileManager.getFullPath(path)) {
        response(400, {message: 'Нельзя удалить корневую папку пользователя.'});
        return;
    }
